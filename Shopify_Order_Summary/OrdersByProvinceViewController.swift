//
//  OrdersByProvinceViewController.swift
...
class OrdersByProvinceViewController: UITableViewController {
	private struct Constants {
		static let ordersByProvinceTitle = "Orders By Province"
		static let ordersByProvinceStatement = " number of orders from "
	...
	private func setupNavigationBarTitle() {
		...
		button.setTitle(Constants.ordersByProvinceTitle, for: .normal)
	...
	override public func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
		if isExpanded(section: section) {
			guard let unwrappedData = data else { return 0 }
			return unwrappedData[section].value.count
		} else {
			return 0
		}
	}
	...
	private func cellText(section: Int) -> String? {
		...
		if isExpanded(section: section) {
			...
			return String(unwrappedData[section].value.count)
				+ Constants.ordersByProvinceStatement
				+ unwrappedData[section].key
		}
	}
}

extension OrdersByProvinceViewController: OrdersByProvinceTableViewHeaderViewDelegate {
